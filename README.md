...
## Using a Custom SSL Keystore with Seamless Upgrade Tool

By default, the Seamless Upgrade Tool uses its own SSL certificate. You can provide a custom certificate by following these steps:

---

### 1️⃣ Create a `certs` Folder

In the directory where you will run the start script, create a `certs` folder:

```bash
mkdir certs
```

All custom SSL files should go in this folder.

---

### 2️⃣ Create a PKCS12 Keystore

Using your SSL certificate and private key, create a `.p12` keystore:
...
* Replace `<keystore-password>` with a password of your choice.
* Make sure the output file is `certs/keystore.p12`.

---

### 3️⃣ Configure Environment Variables

Before running the start script, define these variables:

```bash
export SEAMLESS_UPGRADE_TOOL_TLS_KEY_STORE_PASSWORD=<keystore-password>
export SEAMLESS_UPGRADE_TOOL_TLS_KEY_ALIAS=hyperflex
```

* Replace `<keystore-password>` with the password used in Step 2.

> The tool will automatically pick up the keystore from `./certs/keystore.p12`.

---

### 4️⃣ Start the Tool

Run the tool using the official start script:
